...
public class Test {
    ...
    public static String fileGrade = "GRADE-STUDENT--COURSE.PHU";
                        ...
                        while (bolean) {
                            ...
                            switch (choice1) {
                                ...
                                case 10:
                                    ...
                                    break;
                                case 11:
                                    var checkGrade = grade(input,people,courses);
                                    if(checkGrade) {
                                        saveFileGradeStudentInCourse(courses);
                                    ...
                                    break;
                                case 12:
                                    showlistGradeAndStudent(courses);
                                    break;
                                case 13:
                                    showListStudyStudentInCourse(courses);
                                    break;
                                case 14:
                                    searchStudentOfGradeInCourse(courses,input);
                                    break;
                                case 15:
                                    var index = true;
                                    while(index) {
                                        System.out.println("1.Sắp xếp theo tên của học sinh từ a-z");
                                        System.out.println("2.Sắp xếp theo điểm trung bình kì 1 tăng dần");
                                        System.out.println("3.Sắp xếp theo điểm trung bình kì 1 giảm dần");
                                        System.out.println("4.Sắp xếp theo điểm trung bình kì 2 tăng dần");
                                        System.out.println("5.Sắp xếp theo điểm trung bình kì 2 giảm dần");
                                        System.out.println("6.Sắp xếp theo điểm trung bình của năm tăng dần");
                                        System.out.println("7.Sắp xếp theo điểm trung bình của năm giảm dần");
                                        System.out.println("Khác. Thoát.");
                                        System.out.printf("Mời bạn chọn: ");
                                        String choic1 = input.nextLine();
                                        Sort.index = Integer.parseInt(choic1);
                                        switch (choic1) {
                                            case "1": case "2": case "3": case "4": case "5": case "6": case "7":
                                                sort(input,courses,sorts);
                                                Collections.sort(sorts);
                                                showListSort(sorts);
                                                sorts.removeAll(sorts);
                                                break;
                                            default:
                                                index = false;
                                        }
    ...
    public static void menu() {
        ...
        System.out.println("==  11.Nhập điểm cho từng sinh viên có trong một lớp nào đó  =");
        System.out.println("==  12.Hiển thị bảng điểm của từng lớp học                   =");
        System.out.println("==  13.Bảng kết quả học tập cuối năm.                        =");
        System.out.println("==  14.Tìm kiếm điểm cho bởi mã sinh viên                    =");
        System.out.println("==  15.Sắp xếp");
        System.out.println("==============================================================");
     ...
     * @return true, false;
     */
    public static boolean grade(Scanner input,ArrayList<Person> people, ArrayList<Course> courses) {
        ...
        if(checkIdCourse != null) {
            ...
            for(int i = 0; i < courses.size(); i ++ ) {
                if(courses.get(i).getGradeOfStudents() != null) {
                    for(int j = 0; j < courses.get(i).getGradeOfStudents().size(); j++) {
                        if(idStuden.compareTo(courses.get(i).getGradeOfStudents().get(j).
                                        getStudent().getIdStudent()) == 0) {
                            indexStudent = j;
                        }
                    }
                }
            ...
            if (checkIdStudent != null) {
                ...
                double attend = Double.parseDouble(input.nextLine());
                System.out.println("Điểm giữa kì 1: ");
                double midtermScoreI = Double.parseDouble(input.nextLine());
                System.out.println("Điểm cuối kì 1: " + midtermScoreI);
                double semesterEndGradeI = Double.parseDouble(input.nextLine());

                double avrGradeI = (double)Math.round(grade.calculatorAvgradeII(midtermScoreI, semesterEndGradeI) * 100) / 100;

                System.out.println("Điểm trung bình kì 1: ");

                System.out.println("Điểm giữa kì 2: ");
                double midtermScoreII = Double.parseDouble(input.nextLine());
                System.out.println("Điểm cuối kì 2: ");
                double semesterEndGradeII = Double.parseDouble(input.nextLine());
                double avrGradeII = (double)Math.round(grade.calculatorAvgradeII(midtermScoreII, semesterEndGradeII) * 100) / 100;
                System.out.println("Điểm trung bình kì 2: " + avrGradeII);
                double avGrade = (double)Math.round(((attend * 0.1) + grade.calculatorAvgrade(avrGradeI, avrGradeII)) * 100) / 100;
                System.out.println("Điểm trung bình cả năm: " + avGrade);

                Grade finish =  new Grade(grade.getIdGrade(), attend, midtermScoreI, semesterEndGradeI,
                        avrGradeI, midtermScoreII, semesterEndGradeII, avrGradeII, avGrade);

                checkIdCourse.getGradeOfStudents().set(indexStudent,checkIdCourse.gradeOfStudentInCourse(checkIdStudent,finish));
                return true;
    ...
    public static void saveFileStudentInCourse(ArrayList<Course> courses) {
        try {
            ...
            for(int i = 0; i < courses.size(); i++) {
                if(courses.get(i).getGradeOfStudents() != null) {
                    for (int j = 0; j < courses.get(i).getGradeOfStudents().size(); j++) {
                        printWriter.println(courses.get(i).getGradeOfStudents().get(j));
                    }
                }
            }
    ...
    public static void saveFileGradeStudentInCourse(ArrayList<Course> courses) {
        try {
            FileWriter fileWriter = new FileWriter(fileGrade);
            PrintWriter printWriter = new PrintWriter(fileWriter);
            for(int i = 0; i < courses.size(); i++) {
                if(courses.get(i).getGradeOfStudents() != null) {
                    for(int j = 0; j < courses.get(i).getGradeOfStudents().size(); j++) {
                        var print = courses.get(i).getGradeOfStudents().get(j).toStringGradeOfStudent();
     ...
     */
    public static boolean addStudentToCourse(ArrayList<Course> courses, ArrayList<Person> people,
                                          Scanner input) {
        System.out.println("Mã lớp: ");
        String idCourse = input.nextLine();
        for(int i = 0; i < courses.size(); i++) {
            if(idCourse.compareTo(courses.get(i).getIdCourse()) ==0 ) {
                ...
                courses.get(i).addStudentToCourse(checkIdStudent,new Grade());
                System.out.println("===>Thêm sinh viên thành công<=====");
                return true;
            }
        }
...
//        }
    ...
    public static void showlistGradeAndStudent(ArrayList<Course> courses) {
        for(int i = 0; i < courses.size(); i++) {
            System.out.println("MÃ KHÓA HỌC: " + courses.get(i).getIdCourse());
            System.out.printf("%-14s%-15s%-20s%-14s%-14s%-19s%-19s%-19s%-19s%-19s%-19s%-18s\n",
                    "Mã Bảng điểm","Mã sinh viên", "Họ và tên", "Sinh nhật", "Chuyên cần", "Giữa kì 1",
                    "Cuối Kì 1", "Trung bình kì 1", "Giữa kì 2", "Cuối kì 2", "Trung bình kì 2",
                    "Trung bình năm");
            if (courses.get(i).getGradeOfStudents() != null) {
                for (int j = 0; j < courses.get(i).getGradeOfStudents().size(); j++) {
                    var item = courses.get(i).getGradeOfStudents().get(j).getStudent();
                    var item1 = courses.get(i).getGradeOfStudents().get(j).getGrade();
                    if(item1 != null) {
                        System.out.printf("%-14s%-15s%-20s%-14s%-14s%-19s%-19s%-19s%-19s%-19s%-19s%-18s\n",
                                item1.getIdGrade(),item.getIdStudent(), item.getFullName(), item.getBirth(),
                                item1.getAttendGrade(), item1.getMidtermScoreI(), item1.getSemesterEndGradeI(),
                                item1.getAvGradeI(), item1.getMidtermScoreII(), item1.getSemesterEndGradeII(),
                                item1.getAvGradeII(), item1.getAvGrade());
                    }
                }
            }
            if(i < courses.size() - 1){
    ...
    public static ArrayList<Course> readFileCourse() {
        ...
        try {
            ...
            while(readFile.hasNextLine()) {
                ...
                var checkIdCourse = findOfCourseById(idCourse,courses);
                if(checkIdCourse != null) {
                    checkIdCourse.addStudentToCourse(new Student(idStudent,fullName,birth,gender,major),new Grade());
                }
            }
            readFile.close();
        } catch (FileNotFoundException e) {
        ...
        try {
            ...
            while(readFile.hasNextLine()) {
                var words = readFile.nextLine().split(";");
                var idStudent = words[0];
                var fullName = words[1];
                var birth = words[2];
                var attend = Double.parseDouble(words[3]);
                var midtermScoreI = Double.parseDouble(words[4]);
                var semesterEndGradeI = Double.parseDouble(words[5]);
                var avGradeI = Double.parseDouble(words[6]);
                var midtermScoreII =Double.parseDouble(words[7]);
                var semesterEndGradeII = Double.parseDouble(words[8]);
                var avGradeII = Double.parseDouble(words[9]);
                var avGrade = Double.parseDouble(words[10]);
                var idCourse = words[11];
                var idGrade = words[12];
                int indexStudent = 0;
                var checkIdCourse = findOfCourseById(idCourse,courses);
                if(checkIdCourse != null) {
                  for(int i = 0; i < courses.size(); i++) {
                      if(courses.get(i).getGradeOfStudents() != null) {
                          for (int j = 0; j < courses.get(i).getGradeOfStudents().size(); j++) {
                              if (idStudent.compareTo(courses.get(i).getGradeOfStudents().get(j).getStudent().getIdStudent()) == 0) {
                                    indexStudent = j;
                              }
                          }
                      }
                ...
                Grade grade = new Grade(idGrade,attend,midtermScoreI,semesterEndGradeI,midtermScoreII,
                            semesterEndGradeII,avGradeI,avGradeII,avGrade);
                Student student  = new Student(idStudent,fullName,birth,"","");
                checkIdCourse.getGradeOfStudents().set(indexStudent,checkIdCourse.gradeOfStudentInCourse(student,grade));
            }
        } catch (FileNotFoundException e) {
            throw new RuntimeException(e);
        }
    ...
    public static void showListStudyStudentInCourse(ArrayList<Course> courses) {

        for(int i = 0; i < courses.size(); i++) {
            System.out.println("Mã khóa học: " + courses.get(i).getIdCourse());
            System.out.printf("%-15s%-20s%-16s%-17s%-17s%-17s\n","Mã sinh viên","Họ và tên",
                    "Ngày sinh","Mã Khóa học","Điểm trung bình","Học lực");
            if(courses.get(i).getGradeOfStudents() != null) {
                for(int j= 0 ; j < courses.get(i).getGradeOfStudents().size(); j++) {
                    var item = courses.get(i).getGradeOfStudents().get(j).getStudent();
                    var item1 = courses.get(i).getGradeOfStudents().get(j).getGrade();
                    if (item1 != null) {
                        item1.calculatorStudy(item1.getAvGrade());
                        System.out.printf("%-15s%-20s%-16s%-17s%-17s%-17s\n", item.getIdStudent(), item.getFullName(),
                                item.getBirth(), courses.get(i).getIdCourse(), item1.getAvGrade(), item1.getStudy());
    ...
    /**
     * Phương thức hiển thị điểm cuối năm và xét học lực.
     * @param courses : danh sách các lớp học
     * @param input : nhập mã sinh viên ừ bàn phím
     */
    public static void searchStudentOfGradeInCourse(ArrayList<Course> courses, Scanner input) {
        System.out.println("Nhập mã sinh viên: ");
        String idStudent = input.nextLine();
        for(int i = 0; i < courses.size(); i++) {
            if(courses.get(i).getGradeOfStudents() != null) {
                for(int j = 0; j < courses.get(i).getGradeOfStudents().size(); j++) {
                    if(idStudent.compareTo(courses.get(i).getGradeOfStudents().get(j).getStudent().idStudent) == 0) {
                        var item = courses.get(i).getGradeOfStudents().get(j).getStudent();
                        var item1 = courses.get(i).getGradeOfStudents().get(j).getGrade();
                        System.out.println("+-------------------------------------------------+");
                        System.out.println("|   Sinh viên: " + item.getFullName() +"                   |");
                        System.out.println("+   Mã sinh viên: " + item.getIdStudent()+"                         +");
                        System.out.println("|   Điểm chuyên cần: " + item1.getAttendGrade()+"                          |");
                        System.out.println("+   Điểm giữa kì 1: " + item1.getMidtermScoreI()+"                           +");
                        System.out.println("|   Điểm cuối kì 1: " + item1.getSemesterEndGradeI()+"                           |");
                        System.out.println("+   Điểm trung bình kì 1: " + item1.getAvGradeI()+"                     +");
                        System.out.println("|   Điểm giữa kì 2: " + item1.getMidtermScoreII()+"                           |");
                        System.out.println("+   Điểm cuối kì 2: " + item1.getSemesterEndGradeII()+"                           +");
                        System.out.println("|   Điểm trung bình kì 2: " + item1.getAvGradeII()+"                     |");
                        System.out.println("+   Điểm trung bình cả năm: " + item1.getAvGrade()+"                  +");
                        item1.calculatorStudy(item1.getAvGrade());
                        System.out.println("|   Học lực: " + item1.getStudy().getValue()+"                                 |");
                        System.out.println("+-------------------------------------------------+");
    ...
    public static void sort(Scanner input, ArrayList<Course> courses,ArrayList<Sort> sorts) {
        System.out.println("Nhập mã lớp học: ");
        String idCourse = input.nextLine();
        var checkIdCourse = findOfCourseById(idCourse, courses);
        if (checkIdCourse != null) {
            for (int i = 0; i < checkIdCourse.getGradeOfStudents().size(); i++) {
                var item1 = checkIdCourse.getGradeOfStudents().get(i).getStudent();
                var item2 = checkIdCourse.getGradeOfStudents().get(i).getGrade();
                if(item2 != null) {
                    item2.calculatorStudy(item2.getAvGrade());
                    sorts.add(new Sort(item2.getIdGrade(), item1.getIdStudent(), item1.getFullName(),
                            item1.getBirth(), item2.getAttendGrade(), item2.getMidtermScoreI(), item2.getSemesterEndGradeI(),
                            item2.getAvGradeII(), item2.getMidtermScoreII(), item2.getSemesterEndGradeII(), item2.getAvGradeII(),
                            item2.getAvGrade(), item2.getStudy().getValue()));
                }
            }
        }
    }

    public static void showListSort(ArrayList<Sort> sorts) {
        System.out.printf("%-14s%-15s%-20s%-14s%-14s%-19s%-19s%-19s%-19s%-19s%-19s%-18s%-15s\n",
                "Mã Bảng điểm", "Mã sinh viên", "Họ và tên", "Sinh nhật", "Chuyên cần", "Giữa kì 1",
                "Cuối Kì 1", "Trung bình kì 1", "Giữa kì 2", "Cuối kì 2", "Trung bình kì 2",
                "Trung bình năm", "Học lực");
        for (int j = 0; j < sorts.size(); j++) {
            var item1 = sorts.get(j);
            System.out.printf("%-14s%-15s%-20s%-14s%-14s%-19s%-19s%-19s%-19s%-19s%-19s%-18s%-15s\n",
                    item1.getIdGrade(), item1.getIdStudent(), item1.getFullName(), item1.getBirth(),
                    item1.getAttendGrade(), item1.getMidtermScoreI(), item1.getSemesterEndGradeI(),
                    item1.getAvGradeI(), item1.getMidtermScoreII(), item1.getSemesterEndGradeII(),
                    item1.getAvGradeII(), item1.getAvGrade(),item1.getStudy());
        }
    }
}